// ===========================================================================
// 策略列表 (通过 main 中的 strategy 变量切换):
//   0  = Flat-SIMD baseline        (Lab2 SIMD , 起点)
//   1  = Flat-SIMD + OpenMP        (base 分块并行)
//   2  = Flat-SIMD + Pthread       (手动线程, 用于对比)
//   3  = PQ-Rerank-SIMD baseline   (逐 centroid + AoS, 单线程)
...
static void print_usage(const char* prog) {
    std::cerr << "Usage: " << prog << " [options]\n"
              << "  --strategy   <int>   策略编号 0-18 (default: 4)\n"
              << "  --rerank_k   <int>   PQ rerank 候选数 (default: 500)\n"
              << "  --nprobe     <int>   IVF 搜索簇数 (default: 16)\n"
              << "  --nlist      <int>   IVF 总簇数 (default: 256)\n"
              << "  --omp        <int>   OpenMP 线程数 (default: 4)\n"
              << "  --pthreads   <int>   Pthread 线程数 (default: 4)\n"
              << "  --ef         <int>   HNSW ef_search (default: 64)\n";
}
...
int main(int argc, char* argv[]) {
    ...
    int num_pthreads       = 4;
    int ef_search_arg      = 64;   // 覆盖 hnsw_ns::ef_search

    // ===== 命令行参数解析 =====
    for (int i = 1; i < argc; ++i) {
        std::string a = argv[i];
        if ((a == "--strategy"  || a == "-s") && i+1 < argc) strategy           = std::stoi(argv[++i]);
        else if (a == "--rerank_k"            && i+1 < argc) rerank_k           = std::stoi(argv[++i]);
        else if (a == "--nprobe"              && i+1 < argc) nprobe             = std::stoi(argv[++i]);
        else if (a == "--nlist"               && i+1 < argc) nlist              = std::stoi(argv[++i]);
        else if ((a == "--omp"  || a == "-t") && i+1 < argc) online_omp_threads = std::stoi(argv[++i]);
        else if (a == "--pthreads"            && i+1 < argc) num_pthreads       = std::stoi(argv[++i]);
        else if (a == "--ef"                  && i+1 < argc) ef_search_arg      = std::stoi(argv[++i]);
    ...
    if (strategy >= 16 && strategy <= 18) {
        // ef_search 由命令行 --ef 参数控制
        build_hnsw(base, base_number, vecdim, 16, 200, ef_search_arg);
